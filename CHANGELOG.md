...
- **블래스터 폭/판정 결과 연출 보정**: ① 알테마 블래스터 폭을 **8m→10m**(#1~16 공통, `BLAST_W`) 확대 — 200판 검증상 각 주사위 자리는 여전히 정확히 1대만 피격. ② **실패 시(직선 2대 이상 피격) 빨간 직선 범위를 사라지지 않게 유지**(판정 즉시 `gimmickPhase=failed`로 종료하되 AoE 8개 보존, 봇 정지). ③ **성공 시 전원(봇·플레이어) 가운데 집결**(`gatherToCenter`, `diceGatherPosition`; 공략보기는 키프레임 말미 중앙 프레임으로 동일 연출). 봇 직행 목표를 `diceFinalPosition`으로 분리, 판정 후 위치는 컨트롤러가 일원화. 타임라인에 집결 시간 추가(`DICE_GATHER_MS=4s`, `DICE_TOTAL_MS=판정+4s`).
